...
class MainActivity : AppCompatActivity() {
    ...
    private val chartOptionsResultLauncher =
        registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
            if (result.resultCode == Activity.RESULT_OK) {
                chartOptions =
                    Json.decodeFromString(result.data?.getStringExtra(Constants.NEW_CHART_OPTIONS)!!)
                validateOptions()
            }
        }

    private val datasetOptionsResultLauncher =
        registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
            if (result.resultCode == Activity.RESULT_OK) {
                datasetOptions =
                    Json.decodeFromString(result.data?.getStringExtra(Constants.NEW_DATASET_OPTIONS)!!)
                validateOptions()
            }
        }
